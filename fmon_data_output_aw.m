...
if fmon_task == true
    for ii = 1:length(trialtypes)
        ...
        % Omission Trials 
        if ~isnan(session_data.RawEvents.Trial{1, ii}.States.GoRightOmit)
            right_omit_total = right_omit_total + 1;
            if ~isnan(session_data.RawEvents.Trial{1, ii}.States.CorrectRight)
                right_omit_correct = right_omit_correct + 1;
            end
        end
        if ~isnan(session_data.RawEvents.Trial{1, ii}.States.GoLeftOmit)
            left_omit_total = left_omit_total + 1;
            if ~isnan(session_data.RawEvents.Trial{1, ii}.States.CorrectLeft)
                left_omit_correct = left_omit_correct + 1;
            end
        end
    end
    ...
    left_omit_ratio = ['L Omission: ', num2str(left_omit_correct), '/', num2str(left_omit_total)];
    right_omit_ratio = ['R Omission: ', num2str(right_omit_correct), '/', num2str(right_omit_total)];
...
fprintf(fileID, '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n\n%s\n%s\n%s\n%s\n\n%s\n\n%s\n%s\n\n%s\n%s',...
    ...
    left_ratio, right_ratio, left_omit_ratio, right_omit_ratio);
