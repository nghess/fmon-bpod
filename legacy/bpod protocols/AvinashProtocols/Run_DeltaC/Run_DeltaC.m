...
% GOT HERE ON SEP 25, 2019
else
    S.GUI.MaxTrials = S.MaxTrials;%
end
...
% set case-select defaultsSwitchByState = 'WaitForLick', WaterBypass4NoGo;         %gets reward if licking in GO Trial

BpodSystem.Data.AnalogData = cell(1,MaxTrials);
BpodSystem.Data.StimTypes = ceil(rand(2,MaxTrials)*2);%create 2-row array; row 1 = catch/test; row 2 = lo (950/50) or hi (900/100) odor levels (MFCOut)
...
for currentTrial = 1:S.GUI.MaxTrials
    ...
    sma = AddState(sma, 'Name', 'OpenVial', ... % #2 Open olfactometer vial - wait for MFC flow to stabilize (sma1)
        'Timer', 2,...
        'StateChangeConditions', {'Tup','WaitForStableFlow'},...
        'OutputActions', {'ValveModule2', 1});
    %#3
    sma = AddState(sma, 'Name', 'WaitForStableFlow', ...     % #3 wait 3 s for odor levels to equilibrate
        'Timer', 3,...
        'StateChangeConditions', {'Tup', 'SniffTriggerOn'},...
        'OutputActions', {});
    %#4
    sma = AddState(sma, 'Name', 'SniffTriggerOn', ...            % #4 start monitoring for sniff
        'Timer', 0,...
    ...
    sma = AddState(sma, 'Name', 'EndSniffs', ...%                   % Close all vials and channels
        'Timer', 0,...
        'StateChangeConditions', {'Tup', SwitchByState},...
    ...
    sma = AddState(sma, 'Name', 'DrinkingGrace', ...                %Add 500 ms for mouse to respond or drink
        ...
        'OutputActions', {'ValveModule1',3});bnc
    %#16
